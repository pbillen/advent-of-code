...
insert into path(wire, rank, direction, length)
select    1, path.ordinality, substring(path from 1 for 1), substring(path from 2)::int4
...
union all
select    2, path.ordinality, substring(path from 1 for 1), substring(path from 2)::int4
...
-- We can completely ignore the fact that a wire might intersect with itself. Below, for each such
-- intersection, a duplicate point will be detected, but with different `s` values, denoting the
-- number of steps required to reach that intersection. The remainder of the query will just work,
-- regardless the presence of such duplicates.

with
step(wire, rank, subrank, dx, dy) as
(
    select    path.wire,
              path.rank,
              subrank,
              case when path.direction = 'R' then 1 when path.direction = 'L' then -1 else 0 end,
              case when path.direction = 'U' then 1 when path.direction = 'D' then -1 else 0 end
    from      path, generate_series(1, path.length) as subrank
),
point(wire, x, y, s) as
(
    select    step.wire,
              sum(dx) over(partition by step.wire order by step.rank, step.subrank),
              sum(dy) over(partition by step.wire order by step.rank, step.subrank),
              count(1) over(partition by step.wire order by step.rank, step.subrank)
    from      step
),
intersection(x, y, s1, s2) as
(
    select    point_1.x, point_1.y, point_1.s, point_2.s
    from      point as point_1, point as point_2
    where     point_1.wire < point_2.wire and
              (point_1.x, point_1.y) = (point_2.x, point_2.y)
)
select    min(abs(intersection.x) + abs(intersection.y))
from      intersection
union all
select    min(intersection.s1 + intersection.s2)
from      intersection;
